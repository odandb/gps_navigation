...
///
/// Class to manage navigation with Google Maps
///
class GoogleMaps extends NavigationMap {
  /// Name of the application
  @override
  String get name => "Google Maps";

  ///
  /// Return true if Google Maps is available on the phone, false otherwise
  ///
  @override
  Future<bool> exist() async {
    try {
      await AppAvailability.checkAvailability(DeepLink.googleMaps);
      return true;
    } catch (e) {
      return false;
    }
  ...
  ///
  /// Return a Google Maps query to the destination address
  ///
  String _getAddressQuery(String address) {
    return Uri.encodeFull("${NavigationUrl.googleMaps}$address");
  }
  ...
  String _getLocationQuery(double latitude, double longitude) {
    return Uri.encodeFull("${NavigationUrl.googleMaps}$latitude,$longitude");
  }
}
